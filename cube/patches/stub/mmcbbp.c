...
void dly_us (UINT n);		/* Delay n microseconds (asmfunc.S) */
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2 */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {			/* The card can work at vdd range of 2.7-3.6V */
				for (tmr = 1000; tmr && send_cmd(ACMD41, 1UL << 30); tmr--) dly_us(1000);	/* Wait for leaving idle state (ACMD41 with HCS bit) */
			...
			for (tmr = 1000; tmr && send_cmd(cmd, 0); tmr--) dly_us(1000);	/* Wait for leaving idle state */
